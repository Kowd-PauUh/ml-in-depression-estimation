...
class CNNTrainingModule(L.LightningModule):
    ...
    def _cnn_forward_pass(self, X: torch.Tensor):
        if X.shape[2] != X.shape[3]:
            raise ValueError(
                f'{self.__class__.__name__}._cnn_forward_pass can only take '
                f'tensors of same length and width. Got {X.shape = }'
            )
        return self.cnn(X.to(self.device))

    def _cnn_forward_pass_with_chunking(self, mel_spectrograms: List[torch.Tensor]):
        all_chunks = []
        chunk_lengths = []
    ...
    def _cnn_pooling_forward(self, features: Tuple[torch.Tensor]) -> torch.Tensor:
