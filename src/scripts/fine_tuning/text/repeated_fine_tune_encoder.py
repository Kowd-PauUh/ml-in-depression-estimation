...
from src.training.fine_tuning.encoder_training_module import EncoderTrainingModule
...
EXPERIMENT_NAME = 'Encoder fine-tuning repeated'
...
def repeated_fine_tune_encoder(
    ...
    **kwargs
    ...
    for seed in range(n_repetitions):
        for fold_idx in range(n_folds):
            ...
            # loggers
            csv_logger = CSVLogger(save_dir=MODELS_DIR / model_name_or_path, name=None)
            ...
            mlflow.start_run(
                run_name=f'{model_name_or_path} ("{objective}", {data_leakage=})', 
                tags={
                    'experiment_name': EXPERIMENT_NAME,
                    'cross_validation': timestamp,
                    'objective': objective,
            ...
            trainer = L.Trainer(
                ...
                default_root_dir=model_name_or_path,
                log_every_n_steps=1,
                **kwargs,
            )
...
if __name__ == "__main__":
    fire.Fire(repeated_fine_tune_encoder)
